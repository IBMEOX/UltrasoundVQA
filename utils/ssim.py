...
def ssim(img1, img2, window_size=11, window=None, size_average=True, full=False, val_range=None, map=False):
...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if self.window is None:
            self.window = create_window(self.window_size, image_dim(img1)).to(img1.device).type(img1.dtype)
